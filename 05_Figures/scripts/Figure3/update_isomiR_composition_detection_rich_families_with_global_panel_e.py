...
def terminal_changes(pair_rows: list[dict], no_canonical_rows: list[dict]):
    fasta = read_fasta(CANONICAL_FASTA)
    canonical_by_id = defaultdict(list)
    ...
    for row in pair_rows:
        ...
        expected_id = canonical_name(row["miRNA_ID"])
        canonical = choose_detected_canonical(row, canonical_by_id.get(expected_id, []))
        canonical_source = "detected_canonical_row" if canonical is not None else "expected_canonical_from_fasta"
        if canonical is not None:
            ...
            fasta_id, canonical_seq, fasta_match_type = get_fasta_sequence(fasta, canonical_id)
...
terminal_change_counts, sequence_difference_rows = terminal_changes(pair_rows, no_canonical_rows)
...
# A. isomiR production by mature-arm reported/conservation category.
ax_a = fig.add_subplot(grid[0, 0])
# A mature-arm group is represented by the miRNA name after removing only the
# .v1/.v2 variant suffix. This keeps multiple isomiRs from inflating the arm
# denominator.
locus_records = defaultdict(lambda: {"reported": set(), "conservation": set(), "isomiR": 0})
for row in s2:
    name = str(row.get("miRNA_ID") or "").strip()
    arm = re.sub(r"\.v\d+$", "", name)
    if not arm:
        continue
    record = locus_records[arm]
    record["reported"].add(str(row.get("Reported_Status") or "").lower())
    record["conservation"].add(str(row.get("Conservation") or "").lower())
    if row["Group"] == "isomiR":
        record["isomiR"] += 1
...
summary = {label: {"isomiR": 0, "non-isomiR": 0} for label in category_keys}
for record in locus_records.values():
    reported_label = "reported" if "reported" in record["reported"] else "unreported"
    conservation_label = "conserved" if "conserved" in record["conservation"] else "specific"
    label = f"{reported_label} and\n{conservation_label}"
    if label not in summary:
        continue
    if record["isomiR"] > 0:
        summary[label]["isomiR"] += 1
    else:
        summary[label]["non-isomiR"] += 1

write_tsv(
    DATA_DIR / "isomiR_production_by_mature_arm_category.tsv",
    ["Mature_arm_category", "isomiR_producing_arms", "non_isomiR_arms", "total_arms", "isomiR_percentage", "non_isomiR_percentage"],
    [
        {
            "Mature_arm_category": label.replace("\n", " "),
            "isomiR_producing_arms": summary[label]["isomiR"],
            "non_isomiR_arms": summary[label]["non-isomiR"],
            "total_arms": sum(summary[label].values()),
            "isomiR_percentage": f"{summary[label]['isomiR'] / sum(summary[label].values()) * 100:.1f}",
            "non_isomiR_percentage": f"{summary[label]['non-isomiR'] / sum(summary[label].values()) * 100:.1f}",
        }
        for label in category_keys
    ],
)

x_labels = composition_labels
isomir_counts = np.array([summary[label]["isomiR"] for label in category_keys])
non_isomir_counts = np.array([summary[label]["non-isomiR"] for label in category_keys])
totals = isomir_counts + non_isomir_counts
isomirs = isomir_counts / totals * 100
non_isomirs = non_isomir_counts / totals * 100
x = np.arange(len(x_labels))
ax_a.bar(x, isomirs, width=0.62, color=ORANGE, label="isomiRs")
ax_a.bar(x, non_isomirs, width=0.62, bottom=isomirs, color=BLUE, label="non-isomiRs")
for i, total in enumerate(totals):
    ax_a.text(i, isomirs[i] / 2, f"{isomirs[i]:.1f}%", ha="center", va="center", fontsize=10)
ax_a.set_title("isomiR production by locus category", loc="left", fontsize=13, fontweight="bold", pad=9)
ax_a.set_xticks(x, x_labels)
ax_a.set_ylabel("Proportion of miRNA loci (%)")
...
ax_b.barh(names, reported, color=BLUE, label="Reported miRNAs")
ax_b.barh(names, unreported, left=reported, color=ORANGE, label="Unreported miRNAs")
